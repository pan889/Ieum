 ...
 *
 * ## 이 파일은 **기본 백엔드의 약속**을 붙잡는다
 *
 * `IEUM_SEARCH_BACKEND=opensearch` 로 돌리면 이 중 다섯이 붉어진다. 고장이
 * 아니다 — OpenSearch 백엔드는 사본이고 사본은 몇 초 뒤에 따라온다(실측
 * 4초, ADR-0015). "곧바로" 는 색인이 같은 트랜잭션에 있을 때만 되는 말이고,
 * 그것이 기본값을 Postgres 에 두는 이유다.
 *
 * 그래서 여기에 재시도를 넣지 않는다. 넣으면 이 스펙이 두 백엔드에서 다
 * 초록이 되지만, 그 순간 **더 강한 약속을 붙잡던 시험이 약한 약속을
 * 붙잡는 시험으로 바뀐다.** 기본값의 약속은 "곧바로" 다.
 */
