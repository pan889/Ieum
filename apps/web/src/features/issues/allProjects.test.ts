...
describe('fetchAllProjects', () => {
  it('한 페이지면 한 번만 부른다', async () => {
    ...
    expect((await fetchAllProjects(a)).map((p) => p.key)).toEqual(['A', 'B'])
    expect(calls()).toBe(1)
  })
  ...
  it('커서를 끝까지 따라간다', async () => {
    ...
    expect((await fetchAllProjects(a)).map((p) => p.key)).toEqual(['A', 'B', 'C'])
  })
  ...
  it('빈 결과도 빈 배열이다', async () => {
    const { api: a } = api([[]])
    expect(await fetchAllProjects(a)).toEqual([])
  })
