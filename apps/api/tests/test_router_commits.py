...
NO_WRITE = {
    ...
    # 나오기 때문이다 — 조회가 아니다.
    "mint_collab_ticket",
    # 이관 **미리 보기**는 아무것도 저장하지 않는다. POST 인 것은 묶음
    # 파일을 몸통에 실어야 하기 때문이다. 저장하면 오히려 고장이다 — 짝을
    # 바꿔 가며 몇 번이고 돌려 보는 화면이라, 한 번 본 것만으로 무언가
    # 남으면 두 번째 화면이 첫 번째와 다른 말을 한다
    # (`test_imports_service.py` 가 그것을 고정한다).
    "preview_archive",
}
