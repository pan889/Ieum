...
#: 역할 할당은 **모든 스코프에서** 물을 수 있어야 한다.
#:
#: 스코프 종류가 정의에 없으면 `PermissionService.has` 는 거절이 아니라
#: `ValueError` 를 던진다 — 500 이다. `org.role.assign` 이 global·project 만
#: 받고 있어서, 위키 스페이스나 데스크 큐에 역할을 주려는 요청은 전부 500 이
#: 났다. 스페이스 권한을 줄 길이 **아예 없었다**는 뜻이다(시험은 리포지토리를
#: 직접 불러 스페이스 역할을 만들어서 이 경로를 밟지 않았다).
ANY_SCOPE = frozenset(ScopeKind)
...
registry.register_many(
    [
        ...
        PermissionDef(ROLE_ASSIGN, ANY_SCOPE, "역할 할당", requires_step_up=True),
